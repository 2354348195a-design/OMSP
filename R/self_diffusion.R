#' Self-Diffusion on Local Scaling Affinity
#'
#' This function performs self-diffusion on a given affinity matrix using
#' a diffusion process controlled by eigen-decomposition.
#'
#' @param A A square affinity matrix.
#' @param K Integer. Number of iteration steps used to determine the dominate set.
#'
#' @return A diffused affinity matrix.
#' @keywords diffusion affinity graph
#' @export
#'
#' @examples
#' data(GSE62254_data)
...
#' Compute Dominate Set for Affinity Matrix
#'
#' This function computes the dominate set matrix for a given affinity matrix,
#' based on selecting the top-K nearest neighbors for each row.
#'
#' @param aff.matrix A square affinity matrix.
#' @param NR.OF.KNN Integer. Number of nearest neighbors to retain in each row.
#'
#' @return A symmetric dominate-set matrix with top-K values preserved.
#' @keywords dominate-set knn affinity
#' @export

#' Compute Transition Fields
#'
#' This function converts a given matrix into its transition field representation,
#' using normalization and transformation operations.
#'
#' @param W A square matrix.
#'
#' @return A transformed matrix representing transition fields.
#' @keywords transition-field normalization
#' @export

#' Normalize a Symmetric Kernel Matrix
#'
#' This function normalizes a symmetric kernel matrix using either
#' left-normalization ("ave") or graph Laplacian-style normalization ("gph").
#'
#' @param w A square symmetric matrix.
#' @param type Character string. The normalization type:
#' \itemize{
#'   \item \code{"ave"}: return \eqn{D^{-1} W}
#'   \item \code{"gph"}: return \eqn{D^{-1/2} W D^{-1/2}}
#' }
#'
#' @return A normalized kernel matrix.
#' @keywords normalization kernel
#' @export
